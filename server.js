...
// To accept the request from anywhere we use cors beacuse react is running on diff port and server is on 3000 port
server.use(cors())
...
// To use json format in our server 
...
//Upload url to the server
    const generateUploadURL = async() =>{
       ...
       return await s3.getSignedUrlPromise('putObject',{
            ...
            Bucket: 'blogging-website-project',
            Key: imageName,
            Expires: 1000,
    ...
    //VERIFY JWT for createblog route
    ...
    const formatDatatoSend=(user)=>{
        ...
        const access_token = jwt.sign({id:user._id},process.env.SECRECT_ACCESS_KEY);
    ...
    // if username already exits then add a nanoid to that name or do nothing 
...
server.get("/get-upload-url",(req,res)=>{
    ...
    generateUploadURL().then(url=>
        ...
        res.status(200).json({uploadURL:url})).catch(err=>{
...
// SIGNUP
// Get the Data from frontend and send it back to frontend using res 
...
server.post("/signin",(req,res)=>{          
        ...
        if(!user){
            ...
            // Check if the user has logged in already using the google account 
            // Then theres no need for the password they can login using email itself
        if(!user.google_auth){
    ...
    server.post('/googleauth',async(req,res)=>{
        ...
        getAuth().verifyIdToken(accessToken).then(async(decodedUser)=>{
            ...
            // Find the user and give me all the data in the form of object and return it
            ...
            // LOGIN
            // If we already had signed then we will not allow to signup again with the google
            if(user){
                ...
                bcrypt.hash(newPassword,10 ,(err,hashed_password)=>{
                    ...
                    .then((u)=>{
                        ...
                        return res.status(200).json({status:'password changed'})
                            ...
                            return res.status(500).json({status:'Some error occured while saving new password, please try again later'})
                    })
                })
    ...
    server.post('/create-blog',verifyJWT,(req,res)=>{
        ...
        // Destructure the blog data from the blogEditor
    ...
    .then((blog)=>{
        ...
        if(!likedByUser){
            ...
            .then(data=>{
                ...
                return res.status(500).json({error:err.message1})
            })
